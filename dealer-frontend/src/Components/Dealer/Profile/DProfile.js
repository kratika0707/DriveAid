...
const DProfile = () => {
  ...
  useEffect(() => {
    const fetchdealerDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/dealers/findbyId/${dealerId}`);
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/dealers/changePassword`, {
        dealerId,
        currentPassword,
        newPassword,
      });
  ...
  if (!dealer) {
    return <div>No dealer details found</div>;
  }
    ...
    <div style={{ padding: '20px', maxWidth: '600px', margin: '4%', height: 'auto', borderRadius: '10px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <div style={{ fontSize: '1.25rem', color: 'black', marginBottom: '2%' }}>
          <strong>Name:</strong> {dealer.bname}
        </div>
        <div style={{ fontSize: '1.25rem', color: 'black', marginBottom: '2%' }}>
          <strong>Contact Number:</strong> {dealer.phone}
        </div>
        <div style={{ fontSize: '1.25rem', color: 'black', marginBottom: '2%' }}>
          <strong>Email Id:</strong> {dealer.email}
        </div>
      </div>
